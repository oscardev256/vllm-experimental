...
sys.path.insert(0, "/home/oscar/vllm_isaac/perceptron/src")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("HuggingFace Perceptron Modular Implementation Demo from inside perceptron/main directory.")
    logger.info("=" * 60)

    hf_path = "PerceptronAI/Isaac-0.1"
    #from huggingface import IsaacConfig, IsaacProcessor, IsaacForConditionalGeneration
    #from huggingface import IsaacForConditionalGeneration
    from transformers import Qwen2VLImageProcessorFast
    from isaac_processor_vllm import IsaacProcessor as IsaacProcessor2
    from transformers.models.qwen2.tokenization_qwen2 import Qwen2Tokenizer
    #from transformers.models.isaac.modeling_isaac import IsaacForConditionalGeneration, Qwen3VLProcessor
    #from transformers.models.isaac.modeling_isaac import TransformersIsaacProcessor as IsaacProcessor
    from huggingface import IsaacProcessor, IsaacForConditionalGeneration
    # Load processor and config from the HF checkpoint
    logger.info(f"Loading processor and config from HF checkpoint: {hf_path}")
    #config = IsaacConfig.from_pretrained(hf_path, trust_remote_code=True)
    #qwen3vl_processor = Qwen3VLProcessor.from_pretrained("Qwen/Qwen3-VL-30B-A3B-Instruct")
    #qwen3vl_processor = Qwen3VLProcessor.from_pretrained(hf_path, trust_remote_code=True)
    #isaac_processor = IsaacProcessor2.from_pretrained(hf_path, trust_remote_code=True)
    #qwen3vl_processor = Qwen3VLProcessor(Qwen2VLImageProcessorFast, Qwen2Tokenizer)
    processor = IsaacProcessor.from_pretrained(hf_path, trust_remote_code=True)
    processor.vision_token = "<|image_pad|>"
    ...
    model = IsaacForConditionalGeneration.from_pretrained(hf_path, trust_remote_code=True)
...
# Process the dummy document using chat templates
    ...
    # Convert document to messages format "<|image_pad|>"
    #messages, images = document_to_messages(DUMMY_DOCUMENT, vision_token=config.vision_token)
    print(f"Using the following vision token: {processor.vision_token}")
    messages, images = document_to_messages(DUMMY_DOCUMENT, vision_token=processor.vision_token)
    ...
    with torch.no_grad():
        ...
        print(f"tensor stream into model {tensor_stream}")
